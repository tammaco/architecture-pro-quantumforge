...
def is_malicious_chunk(chunk_text: str) -> bool:
    for pattern in FORBIDDEN_PATTERNS:
        if re.search(pattern, chunk_text, re.IGNORECASE):
            return True
    return False

def filter_malicious_chunks(chunks: list) -> list:
    return [chunk for chunk in chunks if not is_malicious_chunk(chunk["text"])]

def sanitize_prompt(prompt: str) -> str:
    prompt = re.sub(r'(?i)ignore all instructions.*?\n', '', prompt)
    prompt = re.sub(r'(?i)output:.*?\n', '', prompt, flags=re.IGNORECASE)
    return prompt
...
Ответ: Я не знаю"""
    ...
    def ask(self, query: str) -> dict:
        ...
        retrieved = filter_malicious_chunks(retrieved)
